...
SELECT
  (SELECT escolaridade FROM populacao GROUP BY escolaridade ORDER BY COUNT(*) DESC LIMIT 1) AS escolaridade_mais_comum,
  (SELECT ocupacao FROM populacao GROUP BY ocupacao ORDER BY COUNT(*) DESC LIMIT 1) AS ocupacao_mais_comum,
  (SELECT estado FROM populacao GROUP BY estado ORDER BY COUNT(*) DESC LIMIT 1) AS estado_mais_comum,
  (SELECT genero FROM populacao GROUP BY genero ORDER BY COUNT(*) DESC LIMIT 1) AS genero_mais_comum,
  (SELECT estado_civil FROM populacao GROUP BY estado_civil ORDER BY COUNT(*) DESC LIMIT 1) AS estado_civil_mais_comum,
  (SELECT renda_mensal FROM populacao GROUP BY renda_mensal ORDER BY COUNT(*) DESC LIMIT 1) AS renda_mais_comum
;